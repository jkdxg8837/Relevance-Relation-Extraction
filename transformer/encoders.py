...
class CrossAtt(nn.Module):
    ...
    def forward(self, input, side_input, attention_weights=None):
        # input (b_s, seq_len, d_in)
        attention_mask = (torch.sum(side_input, -1) == self.padding_idx).unsqueeze(1).unsqueeze(1)  # (b_s, 1, 1, seq_len)
        encoder_mask = (torch.sum(input, -1) == self.padding_idx).unsqueeze(1).unsqueeze(1)  # (b_s, 1, 1, seq_len)
        out = input
        for l in self.layers:
            out = l(input, side_input, side_input, attention_mask, attention_weights)
            # outs.append(out.unsqueeze(1))
